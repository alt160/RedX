...
namespace TestCode
{
    ...
    public class ElccUser
    {
        ...
        public Signature Sign(string message)
        {
            ...
            // 3) Response: k − priv·c  (mod TerrainSize)
            BigInteger S = (k - _privateKey * c) % _universe.TerrainSize;
            if (S < 0) S += _universe.TerrainSize;
    ...
    public static class Verifier
    {
            ...
            Signature sig)
        {
            ...
            // Reconstruct R:  Jump(0, S + c·pub) == R ?
            BigInteger combined = sig.S + (signerPublicKey * c);
            int expectedR = universe.Jump(0, combined);

            return expectedR == sig.R;
        }
    }

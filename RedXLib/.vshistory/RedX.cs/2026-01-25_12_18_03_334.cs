...
namespace RedxLibx
{
    public static class RE
    {
        ...
        // Backward-compatible overload (no ROK target)
        ...
        /// <summary>
        /// ROK decrypt with mandatory authority verification (anti-symmetric).
        /// Performs a verify pass first (no plaintext emitted) and returns null on any failure.
        /// Rejects compact header encoding for IV/auth in this mode.
        /// </summary>
        ...
        /// <summary>
        /// Convenience wrapper for ROK decrypt that always enforces authority verification.
    ...
    public sealed class REReadOnlyKey
    {
        ...
        public BufferStream UnmapDataWithAuthority(BufferStream mapped, short startLocation, ReadOnlySpan<byte> iv, int checkpointInterval, int checkpointCount, ReadOnlySpan<byte> sigs64xN, ECDsa authorityPublicKey, ReadOnlySpan<byte> rKeyId32, int count = -1)
        {
            ...
            if (checkpointCount > 0)
            {
                ...
                if (sigs64xN.Length < checkpointCount * RE.DefaultAuthoritySigSize)
            ...
            var bx = new JumpGenerator(keyHash.Span, 1, iv);
            Span<byte> obsState = stackalloc byte[RE.ObserverStateSize];
            obsState.Clear();
            uint step = 0;
            int ckRead = 0;

            Span<byte> msg = stackalloc byte[RE.AuthorityDomainBytes.Length + 32 + 4 + RE.ObserverStateSize];
            ...
            for (int i = 0; (count < 0 || i < count) && mapped.Position < mapped.Length; i++)
            {
                ...
                uint h32Landing = RE.ComputeH32(keyHash.Span, landingFlat, noncesSpan[landingFlat]);
                ...
                int flatIndex = curRow * 256 + newCol;
                uint h32 = RE.ComputeH32(keyHash.Span, flatIndex, noncesSpan[flatIndex]);
                if (!map.TryGetValue(h32, out byte plain))
                    throw new CryptographicException($"ROK lookup failed at index {flatIndex}");
                ...
                {
                    int msgLen = RE.BuildAuthorityMessage(rKeyId32, ckRead, obsState, msg);
                    var sig = sigs64xN.Slice(ckRead * RE.DefaultAuthoritySigSize, RE.DefaultAuthoritySigSize);
    ...
    public class REKey
    {
        ...
        public BufferStream MapDataWithObserver(ReadOnlySpan<byte> data, short startLocation, ReadOnlySpan<byte> iv, int checkpointInterval, int checkpointCount, Span<byte> checkpointStates32xN)
        {
            ...
            {
                if (checkpointInterval <= 0) throw new ArgumentOutOfRangeException(nameof(checkpointInterval));
                if (checkpointStates32xN.Length < checkpointCount * RE.ObserverStateSize)
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                {
                    obsState.CopyTo(checkpointStates32xN.Slice(ckWrite * RE.ObserverStateSize, RE.ObserverStateSize));
                    ckWrite++;
                }
            }

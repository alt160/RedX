...
{
    /// <summary>
    /// RedX exposes the public surface for the RedX random-walk cipher with an internal jump stream and a key-row offset stream payload.<br/>
    /// The cipher walks a keyed 2D permutation grid, uses an internal jump stream, and emits a key-row offset stream that is replayed to recover plaintext.<br/>
    ...
    {
        // Lightweight toggle for temporary anti-symmetric tracing; set false to silence.
        internal const bool DebugAntiSym = true;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static void DebugAnti(string message)
        {
            if (DebugAntiSym)
                Console.WriteLine("[anti-debug] " + message);
        }
        ...
        /// <param name="intCat">Header interference catalyst (rokLock for anti-sym headers, empty for symmetric).<br/></param>
        ...
        private static bool TryReadStartLocation1Bit(REKey key, BufferStream ciphertext, ReadOnlySpan<byte> intCat, out ushort startLocation)
        {
            startLocation = 0;
            ...
            try
            {
                ...
                for (; idx < MaxBytes; idx++)
                {
                    ...
                    int rowJump = ((jump % keyLength) / 256);
                    int colJump = jump % 256;
        ...
        /// <summary>
        /// Read a 1-bit encoded start location from a ROK compact header stream.<br/>
        /// Assumes the compact marker (0xFE) and XOR keystream encoding used by MapData(rok, ...).<br/>
        /// Returns false on decode failure, unexpected EOF, or non-canonical encoding.<br/>
        /// </summary>
        /// <param name="rok">Read-only key used to unmask the compact header.<br/></param>
        /// <param name="ciphertext">Ciphertext stream positioned at the startLocation field.<br/></param>
        /// <param name="rokLock">ROK lock bytes for keystream derivation.<br/></param>
        /// <param name="startLocation">Decoded start location value (ushort range).<br/></param>
        /// <returns>True on success; false on failure.<br/></returns>
        private static bool TryReadStartLocation1BitRokCompact(REReadOnlyKey rok, BufferStream ciphertext, ReadOnlySpan<byte> rokLock, out ushort startLocation)
        {
            startLocation = 0;
            if (rok == null || ciphertext == null) return false;
            try
            {
        ...
        /// <summary>
        /// Rehydrate a full key from the serialized blob produced by REKey.ToBytes().<br/>
        /// Provides a frictionless entry point for callers that stay on the RedX static surface.<br/>
        /// </summary>
        public static REKey CreateKeyFromBytes(ReadOnlySpan<byte> serializedKey)
        {
            return REKey.FromBytes(serializedKey);
        }

        /// <summary>
        /// Symmetric encrypt using only the full key.<br/>
        /// Wire layout: enc(startLocation1Bit) | intCatLen | enc(intCat) | key-row offset stream | [enc(integritySeal, optional when useIntegrity=true)].<br/>
        /// Integrity seal is computed over key-row offset stream||intCat; when useIntegrity is false the seal is omitted entirely.<br/>
        /// </summary>
        public static BufferStream Encrypt(byte[] data, REKey key, bool useIntegrity = true)
        {
        ...
        public static BufferStream EncryptAntiSym(byte[] data, REKey key, REReadOnlyKey rokTarget, ReadOnlySpan<byte> authorityPrivateKeyPkcs8, int maxCheckpoints = DefaultAuthorityCheckpointMax, bool useIntegrity = true)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));
            ...
            // 3) encrypted start location (pos-0 mapping; ROK-bound when available)
            var startBuf = startLocation.To1BitEncodedBytes(randomizeUnusedBits: true);
            if (rokTarget != null)
            {
                using var startEnc = key.MapData(rokTarget, rokLockBytes, startBuf, 0);
                ret.Write(startEnc);
            }
            else
            {
                using var startEnc = key.MapData(startBuf, 0, rokLockBytes);
                ret.Write(startEnc);
            }
            ...
            if (useCheckpoints)
            {
                ...
                DebugAnti($"EncryptAntiSym checkpoints: ckCount={ckCount} interval={interval} cipherLen={cipherBytes.Length} useIntegrity={useIntegrity}");
            }
            else
            {
                ...
                DebugAnti($"EncryptAntiSym single-accumulator: ckCount=1 interval={interval} cipherLen={cipherBytes.Length} useIntegrity={useIntegrity}");
            }

            // write encrypted checkpoint count and signatures (control stream encrypted under rokLockBytes at startLocation=0)
            ...
            if (rokTarget != null)
            {
                // Map the interference catalyst header without catalyst-mixing so ROK-side decrypt mirrors the symmetric path.
                using var headerEnc = key.MapData(rokTarget, rokLockBytes, intCat, intCatLen);
                ret.Write(headerEnc);
            }
            else
            {
            ...
            if (useIntegrity)
            {
                ...
                if (rokTarget != null)
                {
                    using var integrityEnc = key.MapData(rokTarget, rokLockBytes, integritySeal, intCatLen, intCat);
                    ret.Write(integrityEnc);
                }
                else
                {
        ...
        /// </summary>
        public static BufferStream EncryptAntiSym(byte[] data, REKey key, ReadOnlySpan<byte> authorityPrivateKeyPkcs8, int maxCheckpoints = DefaultAuthorityCheckpointMax, bool useIntegrity = true)
        {
            return EncryptAntiSym(data, key, null, authorityPrivateKeyPkcs8, maxCheckpoints, useIntegrity);
        }
        ...
        /// </summary>
        public static BufferStream EncryptAntiSym(byte[] data, RedXMintingKey mintingKey, int maxCheckpoints = DefaultAuthorityCheckpointMax, bool useIntegrity = true)
        {
            if (mintingKey == null) throw new ArgumentNullException(nameof(mintingKey));
            var verifier = mintingKey.CreateVerifierKey();
            return EncryptAntiSym(data, mintingKey.FullKey, verifier.Rok, mintingKey.AuthorityPrivateKeyPkcs8, maxCheckpoints, useIntegrity);
        }

        /// <summary>
        /// ROK decrypt with mandatory authority verification (anti-symmetric).<br/>
        /// Wire layout (anti-symmetric): rokLock | enc(startLocation1Bit) | enc(ckCount) | enc(sigs…) | intCatLen | enc(intCat) | key-row offset stream | [enc(integritySeal, optional)].<br/>
        ...
        /// </summary>
        public static BufferStream DecryptAntiSymWithAuthority(BufferStream ciphertext, REReadOnlyKey rok, ReadOnlySpan<byte> authorityPublicKeySpki, int maxCheckpoints = DefaultAuthorityCheckpointMax, bool requireIntegrity = true)
        {
            ...
            if (rokLock.Length != 4) return null;

            if (!TryReadStartLocation1BitRokCompact(rok, ciphertext, rokLock, out ushort startLocationU16))
            {
                DebugAnti("DecryptAntiSym(rok) startLocation decode failed");
                return null;
            }
            ...
            try
            {
                ...
                for (int i = 0; i < ckCount; i++)
                {
                    ...
                    {
                        DebugAnti($"DecryptAntiSym(rok) sig[{i}] null/len!=64");
                        return null;
                    }
                ...
                var intCatLen = ciphertext.ReadByte();
                if (intCatLen < 0) return null;
                var intCat = rok.UnmapData(ciphertext, (short)intCatLen, rokLock, default, intCatLen, rejectCompactHeader: false);
                if (intCat == null || intCat.Length != intCatLen)
                    throw new InvalidDataException("Interference catalyst header decode failed (anti-symmetric ROK).");

                int remaining = (int)(ciphertext.Length - ciphertext.Position);
                // Integrity seal is ROK-mapped; compact encoding adds a 1-byte marker.
                int integrityEncodedLen = requireIntegrity ? 33 : 0; // MapData(rok, ...) uses compact path for 32-byte integrity seal
                ...
                if (requireIntegrity)
                {
                    ...
                    {
                        DebugAnti($"DecryptAntiSym(rok) integrityEnc length mismatch (got {integrityEnc.Length}, want {integrityEncodedLen})");
                        return null;
                    }
                    using var integrityPlain = rok.UnmapData(new BufferStream(integrityEnc), (short)intCatLen, rokLock, intCat.AsReadOnlySpan, 32, rejectCompactHeader: false);
                    if (integrityPlain == null || integrityPlain.Length != 32)
                    {
                        DebugAnti("DecryptAntiSym(rok) integrityPlain null/len!=32");
                        return null;
                    }
                    var b3 = Blake3.Hasher.New();
                    b3.Update(rowOffsetBytes);
                    ...
                    {
                        DebugAnti("DecryptAntiSym(rok) integrity seal mismatch");
                        return null;
                    }
                }
                ...
                {
                    DebugAnti($"DecryptAntiSym(rok) plan mismatch planCount={planCount} ckCount={ckCount}");
                    return null;
                }
                DebugAnti($"DecryptAntiSym(rok) start={startLocationU16} ckCount={ckCount} interval={interval} cipherLen={cipherLen} intCatLen={intCatLen} requireIntegrity={requireIntegrity}");
                ...
                using (var ecdsa = ECDsa.Create())
                {
                    ...
                    var verifyOnly = rok.UnmapDataWithAuthority(new BufferStream(rowOffsetBytes), startLocation, intCat.AsReadOnlySpan, interval, ckCount, sigs, ecdsa, rKeyId32, count: cipherLen);
                    if (verifyOnly == null)
                        return null;
                }
                ...
                var plain = rok.UnmapDataWithAuthority(new BufferStream(rowOffsetBytes), startLocation, intCat.AsReadOnlySpan, interval, ckCount, sigs, ecdsa2, rKeyId32, count: cipherLen);
        ...
        /// Assumes the symmetric wire layout: enc(startLocation1Bit) | intCatLen | enc(intCat) | key-row offset stream | [enc(integritySeal, optional)].<br/>
        ...
        /// </summary>
        public static BufferStream Decrypt(BufferStream ciphertext, REKey key, bool requireIntegrity = true)
        {
            if (ciphertext == null) throw new ArgumentNullException(nameof(ciphertext));
            if (key == null) throw new ArgumentNullException(nameof(key));
            ...
            if (requireIntegrity)
            {
                ...
                Span<byte> integrity2 = stackalloc byte[32];
                b3.Finalize(integrity2);
                using var integrityPlain = key.UnmapData(new BufferStream(integrityBytes), intCatLen, intCat.AsReadOnlySpan, 32);
                if (integrityPlain == null || !integrityPlain.AsReadOnlySpan.SequenceEqual(integrity2))
                    return null;
            }

            var plain = key.UnmapData(new BufferStream(rowOffsetBytes), startLocation, intCat.AsReadOnlySpan);
            plain.Position = 0;
            return plain;
        }
        ...
        /// </summary>
        public static BufferStream DecryptAntiSymWithAuthority(BufferStream ciphertext, RedXVerifierKey verifierKey, int maxCheckpoints = DefaultAuthorityCheckpointMax, bool requireIntegrity = true)
        {
            if (verifierKey == null) throw new ArgumentNullException(nameof(verifierKey));
            return DecryptAntiSymWithAuthority(ciphertext, verifierKey.Rok, verifierKey.AuthorityPublicKeySpki, maxCheckpoints, requireIntegrity);
        }

        /// <summary>
        /// Overload for byte[] ciphertext convenience.<br/>
        /// Wraps the byte array in a BufferStream and forwards to the symmetric decrypt path.<br/>
        /// </summary>
        public static BufferStream Decrypt(byte[] ciphertext, REKey key, bool requireIntegrity = true)
        {
            return Decrypt(new BufferStream(ciphertext), key, requireIntegrity);
        }
    ...
    {
        private const byte RokVersionCompact = 2; // compact seed+key format (current)
        ...
        // decryption
        /// <summary>
        /// Replay a key-row offset stream into plaintext using the ROK.<br/>
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream UnmapData(BufferStream mapped, short startLocation, ReadOnlySpan<byte> rokLock, ReadOnlySpan<byte> intCat = default, int count = -1, bool rejectCompactHeader = false)
        {
        ...
        /// <summary>
        /// Replay a key-row offset stream into plaintext while enforcing authority signatures (ROK context).<br/>
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream UnmapDataWithAuthority(BufferStream mapped, short startLocation, ReadOnlySpan<byte> intCat, int checkpointInterval, int checkpointCount, ReadOnlySpan<byte> sigs64xN, ECDsa authorityPublicKey, ReadOnlySpan<byte> rKeyId32, int count = -1)
        {
            ...
            for (int i = 0; (count < 0 || i < count) && mapped.Position < mapped.Length; i++)
            {
                ...
                {
                    RedX.DebugAnti($"ROK landing lookup failed at index {landingFlat}");
                ...
                if (!map.TryGetValue(h32, out byte plain))
                {
                    RedX.DebugAnti($"ROK lookup failed at index {flatIndex}");
                    throw new CryptographicException($"ROK lookup failed at index {flatIndex}");
                }
                ...
                if (checkpointCount > 0 && ((i + 1) % checkpointInterval) == 0 && ckRead < checkpointCount)
                {
                    ...
                    {
                        RedX.DebugAnti($"ROK authority signature verify failed at checkpoint {ckRead}.");
                        return null;
                    }
                    ckRead++;
                }
            ...
            if (checkpointCount > 0 && ckRead < checkpointCount)
            {
                ...
                if (!authorityPublicKey.VerifyData(msg.Slice(0, msgLen), sig, HashAlgorithmName.SHA256, DSASignatureFormat.IeeeP1363FixedFieldConcatenation))
                {
                    RedX.DebugAnti("ROK authority signature verify failed at final checkpoint.");
                    return null;
                }
            }
    ...
    public class REKey
    {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream MapData(ReadOnlySpan<byte> data, short startLocation, Span<byte> intCat = default)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream MapDataWithObserver(ReadOnlySpan<byte> data, short startLocation, ReadOnlySpan<byte> intCat, int checkpointInterval, int checkpointCount, Span<byte> checkpointStates32xN)
        {
            if (checkpointCount < 0) throw new ArgumentOutOfRangeException(nameof(checkpointCount));
            if (checkpointCount > 0)
            {
        ...
        public BufferStream UnmapDataWithAuthority(BufferStream mapped, short startLocation, ReadOnlySpan<byte> intCat, int checkpointInterval, int checkpointCount, ReadOnlySpan<byte> sigs64xN, ECDsa authorityPublicKey, ReadOnlySpan<byte> rKeyId32, int count = -1)
        {
            ...
            for (int i = 0; (count < 0 || i < count) && mapped.Position < mapped.Length; i++)
            {
                ...
                if (checkpointCount > 0 && ((i + 1) % checkpointInterval) == 0 && ckRead < checkpointCount)
                {
                    ...
                    if (!authorityPublicKey.VerifyData(msg.Slice(0, msgLen), sig, HashAlgorithmName.SHA256, DSASignatureFormat.IeeeP1363FixedFieldConcatenation))
                    {
                        RedX.DebugAnti("Full-key authority signature verify failed during checkpoints.");
                        return null;
                    }
            ...
            if (checkpointCount > 0 && ckRead < checkpointCount)
            {
                ...
                if (!authorityPublicKey.VerifyData(msg.Slice(0, msgLen), sig, HashAlgorithmName.SHA256, DSASignatureFormat.IeeeP1363FixedFieldConcatenation))
                {
                    RedX.DebugAnti("Full-key authority signature verify failed at final checkpoint.");
                    return null;
                }
            }
        ...
        /// <summary>
        /// Map data for a specific ROK target by encoding per-position nonces alongside the key-row offset stream.<br/>
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream MapData(REReadOnlyKey rok, ReadOnlySpan<byte> rokLock, ReadOnlySpan<byte> data, short startLocation, Span<byte> intCat = default)
        {
            ...
            var bx = new JumpGenerator(rok.keyHash.Span, 1, intCat);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream UnmapData(BufferStream mapped, short startLocation, ReadOnlySpan<byte> intCat = default, int count = -1)
        {
            int start = ((ushort)startLocation) % (keyBlockSize * 256);
            var sRow = start / 256;
            var sCol = start % 256;
        ...
        /// <summary>
        /// Convenience overload that accepts an in-memory key-row offset stream instead of BufferStream.<br/>
        /// Useful for tests or callers that already materialized the cipher bytes.
        /// </summary>
        public BufferStream UnmapData(Memory<byte> skipsSpan, short startLocation, ReadOnlySpan<byte> intCat = default, int count = -1)
        {
            return UnmapData(new BufferStream(skipsSpan), startLocation, intCat, count);
        }
        ...
        /// </summary>
        public static REKey FromBytes(ReadOnlySpan<byte> blob)
        {
        ...
        // todo: unused public member (review)
        /// <summary>
        ...
        /// </summary>
        public static REKey FromHex(string hex)
        {
        ...
        /// </summary>
        internal static REKey RehydrateFromRawKeyBytes(ReadOnlySpan<byte> keyBytes)
        {
            ...
            var key = new REKey((byte)blockSize)
            {
    ...
    public sealed class RedXMintingKey
    {
        ...
        private static REKey RehydrateKey(byte keyBlockSize, ReadOnlySpan<byte> keyBytes)
        {
            ...
            var key = REKey.RehydrateFromRawKeyBytes(keyBytes);

...
namespace CodeBasedSignature
{
    ...
    public class CbeSigner
    {
        ...
        public byte[] Sign(ReadOnlySpan<byte> message)
        {
            ...
            // Fold with tail
            return FoldSignature(rawSig, message.Length);
        }
        ...
        {
            // Unfold to raw
            byte[] rawSig = UnfoldSignature(foldedSignature, message.Length);
            // Verify raw
        ...
        /// </summary>
        public byte[] UnfoldSignature (ReadOnlySpan<byte> foldedSignature, int messageLength)
        {
            int foldedLen = messageLength + _noiseLength - 1;
            var folded = foldedSignature.Slice(0, foldedLen);
            var tail = foldedSignature.Slice(foldedLen, _noiseLength);
            var raw = new byte[messageLength * _noiseLength];

            // Recover last block directly from tail
            int lastIndex = messageLength - 1;
            for (int j = 0; j < _noiseLength; j++)
                raw[lastIndex * _noiseLength + j] = tail[j];

            // Reconstruct earlier blocks backwards
            for (int i = messageLength - 2; i >= 0; i--)
            {
                for (int j = 0; j < _noiseLength; j++)
                {
                    int pos = i + j;
                    byte v = folded[pos];
                    // Subtract contributions from all later blocks k > i
                    for (int k = i + 1; k < messageLength; k++)
                    {
                        int offset = pos - k;
                        if (offset >= 0 && offset < _noiseLength)
                            v ^= raw[k * _noiseLength + offset];
                    }
                    raw[i * _noiseLength + j] = v;
                }
            }

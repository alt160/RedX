...
namespace RedxLibx
{
    public static class RE
    {
        ...
        public static BufferStream EncryptAntiSym(byte[] data, REKey key, ReadOnlySpan<byte> authorityPrivateKeyPkcs8, int maxCheckpoints = DefaultAuthorityCheckpointMax)
        {
            ...
            // 4) compute checkpoint plan for the payload steps
            ComputeCheckpointPlan(data.Length, maxCheckpoints, out int ckCount, out int interval);

            // 5) map payload while producing checkpoint observer states
            byte[] ckStates = ckCount > 0 ? new byte[ckCount * ObserverStateSize] : Array.Empty<byte>();
            using var cipher = key.MapDataWithObserver(data, startLocation, iv, interval, ckCount, ckStates);

            // 6) sign each checkpoint state with authority private key

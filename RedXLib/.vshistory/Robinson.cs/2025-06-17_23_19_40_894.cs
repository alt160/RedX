...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static BufferStream Encrypt(byte[] data, REKey key)
        {
            ...
            var headerDec = key.UnmapData(headerEnc);
        ...
        public static BufferStream Decrypt(BufferStream ciphertext, REKey key)
        {
            ...
            var iv = key.UnmapData(ciphertext, default, ivLen);

            var plain = key.UnmapData(ciphertext, iv.AsReadOnlySpan);
    ...
    public class REKey
    {
        ...
        public BufferStream UnmapData(BufferStream mapped, ReadOnlySpan<byte> iv = default, int count = -1)
        {
            // 1) Read the two‐byte header you wrote in MapData
        ...
        public BufferStream UnmapData(Memory<byte> skipsSpan, ReadOnlySpan<byte> iv = default, int count = -1)
        {
            return UnmapData(new BufferStream(skipsSpan), iv, count);
        }

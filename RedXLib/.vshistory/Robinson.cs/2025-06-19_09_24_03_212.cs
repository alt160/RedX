    ...
    public static class RE
    {
        ...
        public static BufferStream Encrypt(byte[] data, REKey key, bool authenticated = false)
        {
            ...
            if (authenticated)
            {
                ...
                ret.Write7BitInt((int)authEnc.Length);
                ret.Write(authEnc);
        ...
        public static BufferStream Decrypt(BufferStream ciphertext, REKey key)
        {
            ...
            if (mode == 1)
            {
                var authLen = (short)ciphertext.Read7BitInt();
                var auth = key.UnmapData(ciphertext, ivLen, iv.AsReadOnlySpan, authLen);
                var b3 = Blake3.Hasher.New();
                b3.Update(ciphertext.AsReadOnlySpan);

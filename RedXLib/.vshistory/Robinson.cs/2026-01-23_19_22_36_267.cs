...
namespace RedxLib
{
    public static class RE
    {
        ...
        public static bool VerifyAuthority(BufferStream ciphertext, REReadOnlyKey rok, ReadOnlySpan<byte> vrfPublicKey, IVrfVerifier verifier, int proofPlainSize = DefaultProofPlainSize, int chunkSize = DefaultAuthorityChunkSize)
        {
            ...
            // read startLocation and rokLock
            var startLocation = (short)bs.Read7BitInt();
            var rokLock = bs.ReadBytes(4);

            // decrypt encrypted count (startLocation used as 0 when encrypting)
            long posBeforeCount = bs.Position;
            int proofCount = 0;
            var proofs = new List<ReadOnlyMemory<byte>>();
            try
            {
                var cntPlain = rok.UnmapData(bs, (short)0, rokLock, default, 4);
                if (cntPlain != null && cntPlain.Length == 4)
                {
                    int pc = MemoryMarshal.Read<int>(cntPlain.AsReadOnlySpan);
                    // debug
                    Console.WriteLine($"[VerifyAuthority] probed proofCount={pc}");
                    // sanity check the count
                    if (pc >= 0 && pc <= 1024)
                    {
                        proofCount = pc;
                        proofs = new List<ReadOnlyMemory<byte>>(proofCount);
                        for (int i = 0; i < proofCount; i++)
                        {
                            var p = rok.UnmapData(bs, (short)0, rokLock, default, proofPlainSize);
                            var pArr = p.ToArray();
                            proofs.Add(pArr);
                            Console.WriteLine($"[VerifyAuthority] decrypted proof[{i}] (base64): {Convert.ToBase64String(pArr)}");
                        }
                    }
                    else
                    {
                        // treat as no proofs; rewind to before the count
                        bs.Position = posBeforeCount;
                        proofCount = 0;
                    }
                }
                else
                {
                    // nothing to read; rewind
                    bs.Position = posBeforeCount;
                }
            }
            catch (Exception ex)
            {
                // on any failure reading proofs, assume none and rewind
                Console.WriteLine($"[VerifyAuthority] probe/unmap failed: {ex.GetType().Name}: {ex.Message}");
                bs.Position = posBeforeCount;
                proofCount = 0;
            }

            // decrypt IV header and auth as usual
            var ivLen = bs.ReadByte();
            var iv = rok.UnmapData(bs, ivLen, rokLock, default, ivLen);
            var authEnc = rok.UnmapData(bs, ivLen, rokLock, iv.AsReadOnlySpan, 32);

            // prepare a JumpGenerator for replaying walk outputs
            var bx = new JumpGenerator(rok.keyHash.Span, 1, iv.AsReadOnlySpan);
            ...
            while (bs.Position < bs.Length)
            {
                ...
                hasher.Update(tmp);
                hasher.Update(iv.AsReadOnlySpan);
                ...
                var proof = proofs[chunkIndex].Span;

    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<short, byte> kStar) MapData(ReadOnlySpan<byte> data)
        {
            ...
            for (int dataIndex = 0; dataIndex < data.Length; dataIndex++)
            {
                ...
                // Optionally apply counter shift to resist repetition patterns
                // b = (byte)((b + dataIndex) % 256); // ← uncomment if desired
        ...
        public BufferStream UnmapData(BufferStream skips, Dictionary<short, byte> kStar)
        {
            ...
            // Read the initial position from the skip stream
            short curPos = skips.ReadInt16();

            int blockSize = 256;
            int blockCount = key.Length / blockSize;
            Span<byte> rotations = stackalloc byte[blockCount];

            while (skips.Position < skips.Length)
            {
                // Read the next skip distance
                int distance;
                byte skipByte = skips.ReadByte();
                if (skipByte == 0)
                    distance = skips.Read7BitInt();
                else
                    distance = skipByte;
                ...
                // Look up the byte stored at this position
                if (!kStar.TryGetValue(curPos, out byte b))
                    throw new InvalidDataException($"Missing mapping for key index {curPos}");

                output.WriteByte(b);

                // Advance the block's virtual rotation
                int curBlock = curPos / blockSize;
                rotations[curBlock] = (byte)((rotations[curBlock] + CalcRotation(distance)) % 256);
            }

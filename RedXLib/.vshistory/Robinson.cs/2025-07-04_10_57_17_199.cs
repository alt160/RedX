...
namespace RobinsonEncryptionLib
{
    ...
    public sealed class REReadOnlyKey
    {
        ...
        private readonly Memory<byte> keyHash;        // 64-byte Blake3 digest
        ...
        public BufferStream UnmapData(            BufferStream mapped,            short startLocation,            ReadOnlySpan<byte> iv = default,            int count = -1)
        {
            ...
            var chPubLocal = SmallPrimeChameleon.CreateFromPublicParam(chPublicParam, this.keyLength);
            var noncesSpan = nonces.Span;
            var map = chMap;
            var hashBuf = stackalloc byte[4];
            var mappedStream = mapped;             // avoid repeated property access

            for (int i = 0;
                 (count < 0 || i < count) && mappedStream.Position < mappedStream.Length;
                 i++)
            {
                int dist = mappedStream.ReadByte();
                if (dist < 0) break;

                // replay skip
                ushort skip = bx.NextJump16();
                int rowJump = (skip % keyLength) / 256;
                int colJump = skip % 256;

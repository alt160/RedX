...
namespace RobinsonEncryptionLib
{
    ...
    public class REReadOnlyKey
    {
        ...
        public REReadOnlyKey(REKey key, ReadOnlySpan<byte> usageKey)
        {
            var b3 = Blake3.Hasher.New();
            b3.Update(key.key.AsSpan());
            var keyHash = b3.Finalize();

            if (!usageKey.IsEmpty)
                b3 = Blake3.Hasher.NewKeyed(usageKey);
            else
            ...
            {
                b3.Reset();
                b3.Update(keyHash.AsSpan());

                iTemp[0] = i;
                b3.Update(bTemp);

                var hash = b3.Finalize();

                for (int offset = 0; offset <= 28; offset++)
                {
                    var hash32 = MemoryMarshal.Read<uint>(hash.AsSpan().Slice(offset, 4));
                    if (keyMap.TryAdd(hash32, key.key[i]))
                    {
                        break;
                    }
                    else if (offset == 28)
        ...
        public REReadOnlyKey(Span<byte> rok, ReadOnlySpan<byte> usageKey)
        {
            var keyHashSize = rok[0];
            var keyHash = rok.Slice(1, keyHashSize);
            var rokData = MemoryMarshal.Cast<byte, KeyMapEntry>(rok.Slice(keyHashSize + 1));
            ...
            for (int i = 0; i < rokData.Length; i++)
            {
                    if (keyMap.TryAdd(rokData[i].hash, rokData[i].value))
                    {
                        break;
                    }
                    else if (offset == 28)
        ...
        {
            var ret = new REReadOnlyKey(null, default);
            ret.keyMap = new Dictionary<uint, byte>();
            for (int i = 0; i < keyBytes.Length; i += 5)
            {
                ret.keyMap.Add(MemoryMarshal.Read<uint>(keyBytes.AsSpan().Slice(i, 4)), keyBytes[i + 4]);
            }
            return ret;
        }
        ...
        public byte[] ToBytes()
        {
            var retBuff =  new byte[keyMap.Count * 5];
            var retSpan = retBuff.AsSpan();
            var idx = 0;
            ...
            foreach (var kv in keyMap.OrderBy(x => x.Value))
            {
                ...
                kTemp[0] = kv.Key;
                bTemp.CopyTo(retSpan.Slice(idx));
                idx += 4;
                retSpan[idx++] = kv.Value;
            }
            return retBuff;
        }
    }
    ...
    public class REKey
    {
        ...
        public REReadOnlyKey CreateReadOnlyKey(ReadOnlySpan<byte> usageKey = default)
        {
            return new REReadOnlyKey(this, default);
        }

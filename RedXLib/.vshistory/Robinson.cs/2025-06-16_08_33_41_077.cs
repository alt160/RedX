    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<short, byte> kStar) MapData(ReadOnlySpan<byte> data)
        {
            ...
            for (int dataIndex = 0; dataIndex < data.Length; dataIndex++)
            {
                ...
                var rkdIndexes = rkd.AsSpan(b, _keySize);
                ...
                {
                    target = rkdIndexes[rkdIndex];

                    if (target > curPos)
                    {
                        distance = target - curPos;
                        goto Emit;
                    }
                }

                // If no forward found, wrap to first
                target = rkdIndexes[0];
...
Emit:
        ...
        public BufferStream UnmapData(BufferStream skips, Dictionary<short, byte> kStar)
        {
            ...
            while (skips.Position < skips.Length)
            {
                ...
                if (!kStar.TryGetValue(curPos, out var value))
                    throw new InvalidOperationException($"Missing value at position {curPos} in kStar.");

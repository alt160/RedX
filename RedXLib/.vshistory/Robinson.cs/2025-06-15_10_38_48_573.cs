...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static byte[] Encrypt(byte[] data, REKey keyA, REKey keyB)
        {
            ...
            var t2 = keyB.CreateDataMap(kStarBuffer.AsReadOnlySpan);
            ...
            // calc sha256 of prefix
            var prefixSha256 = SHA256.HashData(prefix);


            // encrypt prefix with keyB
            var prefixSpan = prefix.AsSpan();
            var pfx = keyB.CreateDataMap(prefixSpan);

            // write ciphertext
            outBuf.Write7BitEncodedInt(prefix.Length);
            outBuf.WriteBytes(prefix);
            outBuf.Write7BitEncodedInt((int)t2.skips.Length);
            t2.skips.CopyTo(outBuf);
            t1.skips.CopyTo(outBuf);

            return outBuf.ToArray();
        }

        public static byte[] Decrypt(byte[] cipher, REKey key )
        {
            // this overload is the asymmetric decryption requiring the kStar values are embedded in the cipher text.

            // map bufferstream over cipher bytes
            var cipherBuf = new BufferStream(cipher);

            // return buffer
            var outBuf = new BufferStream();

            // read prefix length
var prefixLen = cipherBuf.Read7BitEncodedInt();

            // read prefix
            var prefix = cipherBuf.ReadBytes(prefixLen);
    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<short, byte> kStar) CreateDataMap(ReadOnlySpan<byte> data)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                curPos = chosenIdx;
                System.Diagnostics.Debug.WriteLine($"{curPos}|{skipDistance}", "DrawPath::curPos");
        ...
        public byte[] FollowPath(BufferStream skips, Dictionary<short, byte> kStar)
        {

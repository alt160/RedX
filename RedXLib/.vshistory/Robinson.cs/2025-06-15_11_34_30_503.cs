...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static byte[] Encrypt(byte[] data, REKey keyA, REKey keyB)
        {
            ...
            var prefixSha256 = SHA256.HashData(outBuf.AsReadOnlySpan);
            ...
            return outBufMappedByKeyB.skips.ToArray();
        }
        ...
        public static byte[] Decrypt(byte[] ciphertext, REKey keyB)
        {
            ...
            // read prefix length
            var prefixLength = cipherStream.Read7BitEncodedInt();

            // read prefix as slice
            var prefix = cipherStream.SliceAtCurrent(prefixLength);

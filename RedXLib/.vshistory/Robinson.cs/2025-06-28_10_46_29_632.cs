...
namespace RobinsonEncryptionLib
{
    ...
    public class REKey
    {
        ...
        public BufferStream MapData(REReadOnlyKey rok, ReadOnlySpan<byte> data, short startLocation, Span<byte> iv = default)
        {
            ...
            // 1) Build a keyed BLAKE3 hasher
            Span<byte> jumpSeed = stackalloc byte[32];
            {
                var blake = Blake3.Hasher.NewKeyed(rok.seed.Span);

                // 2) Feed in the per-message salt
                blake.Update(iv);

                // 3) Produce a 32-byte jump seed
                blake.Finalize(jumpSeed);
            }

            var bx = new Blake3XofReader(jumpSeed, iv);

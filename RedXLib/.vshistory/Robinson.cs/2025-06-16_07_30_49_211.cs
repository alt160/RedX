...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static byte[] Decrypt(byte[] ciphertext, REKey keyB)
        {
            ...
            var computedHash = SHA256.HashData(encryptedData);
    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<short, byte> kStar) MapData(ReadOnlySpan<byte> data)
        {
            ...
            for (int dataIndex = 0; dataIndex < data.Length; dataIndex++)
            {
                ...
                // there's a case on repeating input values of a ping-pong between backward and forward indexes.  need to adjust.
                ...
                if (bestFwd < 256)
                {
                    ...
                    chosenIdx = rkdIndexes[bestFwdJ];
                }
                else
                {
                    skipDistance = (short)-bestBwd;
                    chosenIdx = rkdIndexes[bestBwdJ];
                }

                if (skipDistance < 0 || skipDistance > 255)
                {
                    skips.Write((byte)0);
                    skips.Write7BitInt(skipDistance);
                }
                else
                ...
                //Debug.WriteLine($"{b}[{(char)b}]={chosenIdx}, {skipDistance}", "kStar Value: ");
            }
            //Debug.WriteLine("");
            //Debug.WriteLine("");
            return (skips, kStar);
        ...
        public byte[] UnmapData(Memory<byte> skipsSpan)
        {
            ...
            while (skips.Position < skips.Length)
            {
                try
                {
                    ...
                    result.Add(key.Span[currentPos]);
                }
                catch (EndOfStreamException)
                {
                    // We've reached the end of the stream
        ...
        public byte[] UnmapData(BufferStream skips, Dictionary<short, byte> kStar)
        {
            ...
            while (skips.Position < skips.Length)
            {
                try
                {
                    ...
                    // Apply the skip to the current position, handling wrap-around
                    currentPos = (short)((currentPos + skip + key.Length) % key.Length);
                    ////Debug.WriteLine($"{currentPos}, {skip}={(char)kStar[currentPos]}", "Unmap currentPos: ");

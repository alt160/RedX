...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static BufferStream Encrypt(byte[] data, REKey key, bool authenticated = false)
        {
            ...
            using var headerEnc = key.MapDataSymmetric(iv, ivLen);
            ...
            using var cipher = key.MapDataSymmetric(data, startLocation, iv);
            ...
            if (authenticated)
            {
                ...
                b3.Finalize(auth);
                using var authEnc = key.MapDataSymmetric(auth, ivLen, iv);
                ret.Write(authEnc);
    ...
    public class REKey
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream MapDataSymmetric(ReadOnlySpan<byte> data, short startLocation, Span<byte> iv = default)
        {
            // wrap startLocation
        ...
        public BufferStream MapDataAntiSymmetric(ReadOnlySpan<byte> data, short startLocation, Span<byte> iv = default)
        {
            // wrap startLocation
            startLocation = (short)(startLocation % keyLength);
            var sRow = startLocation / 256 % keyBlockSize;

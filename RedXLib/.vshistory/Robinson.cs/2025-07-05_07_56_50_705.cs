...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static BufferStream Encrypt(byte[] data, REKey key)
        {
            ...
            uint rokLock = MemoryMarshal.Read<uint>( RandomNumberGenerator.GetBytes(4).AsSpan());
            ret.Write(rokLock);
            // ★ END NEW SEED
            // ────────────────────────────────────────────────────────
        ...
        public static BufferStream Decrypt(BufferStream ciphertext, REKey key)
        {
            ...
            var rokLock = ciphertext.ReadUInt32();
    ...
    public sealed class REReadOnlyKey
    {
        ...
        public BufferStream UnmapData(BufferStream mapped, short startLocation, uint rokLock, ReadOnlySpan<byte> iv = default, int count = -1)
        {
    ...
    public class REKey
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BufferStream MapData(
            REReadOnlyKey rok,
            ReadOnlySpan<byte> data,
            short startLocation,
            Span<byte> iv = default)
        {

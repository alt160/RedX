...
namespace RobinsonEncryptionLib
{
    ...
    public class REReadOnlyKey
    {
        ...
        public REReadOnlyKey(REKey key)
        {
            ...
            var keyMapTemp = new Dictionary<uint, byte>();
            ...
            for (int i = 0; i < key.key.Length; i++)
            {
                ...
                var hash = b3.Finalize().AsSpan();

                for (int offset = 0; offset <= 28; offset++)
                {
                    var hash32 = MemoryMarshal.Read<uint>(hash.Slice(offset, 4));
                    Debug.WriteLine($"{hash32} | {i}", "Hash | Index");
                    if (keyMapTemp.TryAdd(hash32, key.key[i]))
                    {
                        break;
                    }
                    else if (offset == 28)
            ...
            keyMap = new Dictionary<uint, byte>(keyMapTemp);
            foreach (var kv in keyMapTemp.OrderBy(x => x.Value))
                keyMap.Add(kv.Key, kv.Value);
        }

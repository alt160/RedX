    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<byte, short> kStar) DrawPath(ReadOnlySpan<byte> data)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                curPos = chosenIdx;
                System.Diagnostics.Debug.WriteLine(curPos, "DrawPath::curPos");
        ...
        public byte[] FollowPath(BufferStream skips, Dictionary<byte, short> kStar)
        {
            ...
            // pivot kStar making values keys and keys values
var kStarD = new Dictionary<short, byte>();
            foreach (var kv in kStar)
            {
                kStarD[kv.Value] = kv.Key;
            }
            ...
            while (skips.Position < skips.Length)
            {
                try
                {
                    ...
                    System.Diagnostics.Debug.Write((char)kStarD[currentPos]);
                    // Find the data byte corresponding to this key byte using kStar
                    result.Add(kStarD[currentPos]);
                }

...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static BufferStream Encrypt(byte[] data, REKey key)
        {
            ...
            var headerDec = key.UnmapData(headerEnc, ivLen);

            headerEnc.Position = 0;
        ...
        {
            var startLocation = ciphertext.Read7BitInt();
            var ivLen = ciphertext.ReadByte();

            var iv = key.UnmapData(ciphertext, ivLen, default, ivLen);

            var plain = key.UnmapData(ciphertext, (short)startLocation, iv.AsReadOnlySpan);
    ...
    public class REKey
    {
        ...
        public BufferStream UnmapData(BufferStream skips, short startLocation, ReadOnlySpan<byte> iv = default, int count=-1)
        {
            // Normalize startLocation within key space
            startLocation = (short)(startLocation % (_keySize * 256));

            var sRow = startLocation / 256;
            var sCol = startLocation % 256;

            var output = new BufferStream(); // optional: reuse length estimate
            var curRow = sRow;
            var curCol = sCol;
            var ivLen = iv.Length;
            ...
            {
                if (count>0 && output.Length == count) break;

                var dist = skips.ReadByte();
                if (dist < 0)
                    break;

                // Calculate the actual column (forward wrap)
                var col = (curCol + dist) % 256;
                curCol = col;

                // Lookup the byte at this column in the current row (reverse of rkd)
                byte value = key[(curRow * 256) + col];

                // Undo transformation from MapData
                var i = (int)output.Position;
                if (ivLen > 0)
                    value = (byte)((256 + value - i - iv[i % ivLen]) % 256);
                else

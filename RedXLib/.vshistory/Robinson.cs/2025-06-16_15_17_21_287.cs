...
namespace RobinsonEncryptionLib
{
    public static class RE
    {
        ...
        public static BufferStream Decrypt(byte[] ciphertext, REKey keyB)
        {
            ...
            var aStar = new Dictionary<ushort, byte>();
            for (int i = 0; i < aStarCount; i++)
            {
                var index = decryptedStream.ReadUInt16();
    ...
    public class REKey
    {
        ...
        public (BufferStream skips, Dictionary<ushort, byte> kStar) MapData(ReadOnlySpan<byte> data)
        {
            ...
            var kStar = new Dictionary<ushort, byte>(data.Length);

            // Write the initial position to the output
            skips.Write((short)sl);

            // Initialize current position
            var curPos = (ushort)sl;
            ...
            for (int dataIndex = 0; dataIndex < data.Length; dataIndex++)
            {
                ...
                if (target == -1)
                {
                    ...
                    if (rawIndex == curPos)
                        rawIndex = rkdIndexes[1]; // pick next one instead

                    target = rawIndex;
                    distance = (target - curPos + key.Length) % key.Length;
                }
                ...
                // Advance cursor
                curPos = (ushort)((curPos + distance) % key.Length);
        ...
        public BufferStream UnmapData(BufferStream skips)
        {
            ...
            while (skips.Position < skips.Length)
            {
                ...
                // Retrieve value directly from key
                byte encoded = key[curPos];
        ...
        {
            var skips = new BufferStream(skipsSpan); // Wrap the input span
            var output = new BufferStream();
        ...
        public BufferStream UnmapData(BufferStream skips, Dictionary<ushort, byte> kStar)
        {
            // Output buffer for reconstructed plaintext
            var output = new BufferStream();

            // Initial cursor position
            var curPos = skips.ReadUInt16();

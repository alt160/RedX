    ...
    foreach (var chunk in chunkSizes)
    {
        foreach (var psize in proofSizes)
        {
            foreach (var L in plainLens)
            {
                ...
                var cAuth = RE.EncryptWithAuthority(plain, fullKey, rok, proofs, psize);
...
var cWith = RE.EncryptWithAuthority(plainSample, fullKey, rok, proofs, proofSize);
var cb2 = cWith.ToArray();

// Decrypt and verify
var dec = RE.Decrypt(new BufferStream(cb2), rok);
bool decOk = dec != null && Encoding.UTF8.GetString(dec.AsReadOnlySpan) == "abc";
var verifier = new MockVrfVerifier(proof);
bool verified = RE.VerifyAuthority(new BufferStream(cb2), rok, ReadOnlySpan<byte>.Empty, verifier, proofSize, 4096);

var wrongProof = new byte[proofSize]; wrongProof[0] = 0xFF;
var badVerifier = new MockVrfVerifier(wrongProof);
bool verifiedBad = RE.VerifyAuthority(new BufferStream(cb2), rok, ReadOnlySpan<byte>.Empty, badVerifier, proofSize, 4096);

...
string[] infoKeysContain = new[]
{
    ...
    "verifier (ROK) : key hash + per-position nonces + lookup map.",
...
WriteLineColor(ConsoleColor.Cyan, "Mint/Verify mode");
        ...
        if (choice == "ESC" || string.Equals(choice, "X", StringComparison.OrdinalIgnoreCase)) return;
    if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase)) ShowInfo("Mint/Verify: name and meaning", infoAntiSymName);
    else if (string.Equals(choice, "B", StringComparison.OrdinalIgnoreCase)) ShowInfo("Mint/Verify use cases", infoAntiSymUseCases);
    else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase)) ShowInfo("Mint/Verify limits", infoAntiSymLimits);
...
void RunGlossaryWireMenu()
{
    while (true)
    {
        ...
        else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase)) ShowInfo("Wire format: mint/verify", infoWireAntiSym);
...
bool HasAntiPlain() => lastMintPlainHex != null;
bool HasAntiCipher() => lastMintCipherHex != null;
...
/// </summary>
void SelectMintVerifyKeys(ref RedXMintingKey minting, ref RedXVerifierKey verifier)
{
    while (true)
    {
        Console.WriteLine();
        ...
        if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase))
        {
            var pair = REAnti.CreateMintingKeyPair();
...
/// </summary>
void RunMintVerifyPreset(RedXMintingKey minting, RedXVerifierKey verifier, bool useIntegrity)
{
...
/// </summary>
void MintInteractive(RedXMintingKey minting, RedXVerifierKey verifier, bool useIntegrity)
{
    var plain = PromptPayloadBytes("plaintext");
    if (plain == null) return;
...
/// </summary>
void VerifyInteractive(RedXVerifierKey verifier, bool requireIntegrity)
{
    var ctBytes = PromptPayloadBytes("ciphertext (hex preferred)");
    if (ctBytes == null) return;
...
void RunMintVerifyMenu()
{
    ...
    if (showMintVerifyIntro)
        ShowAntiSymIntro(ref showMintVerifyIntro);
    ...
    WriteLineColor(ConsoleColor.Cyan, "Mint/Verify menu (mint/verify):");
        ...
        if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase))
        {
            ...
            RunMintVerifyPreset(minting, verifier, useIntegrityAnti);
        }
        else if (string.Equals(choice, "B", StringComparison.OrdinalIgnoreCase))
        {
            ...
            verifier ??= minting.CreateVerifierKey();
            MintInteractive(minting, verifier, useIntegrityAnti);
        }
        else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase))
        {
            if (verifier == null)
            ...
            VerifyInteractive(verifier, useIntegrityAnti);
        }
    }
}
...
void RunRedXPrimer()
{
    ...
    Console.WriteLine("  A) Mint/Verify (formerly anti-symmetric) - Updated");
        Console.WriteLine($"  ?) {GlossaryLabel}");

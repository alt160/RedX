...
const string GlossaryContextAntiSymmetric = "anti-symmetric";
const string GlossaryContextSymmetricKey = "symmetric-key";
const string GlossaryContextAntiSymmetricKey = "anti-symmetric-key";
...
string[] infoCorePathWalking = new[]
{
    ...
    "This [[path-walking]] design is the root of symmetric and [[Anti-symmetric mode]] behavior."
};
...
string[] infoPrimerIntro = new[]
{
    ...
    "Ciphertext records a keyed traversal across a 2D permutation grid; decryption replays that walk to recover plaintext.",
    "This primer focuses on usage and capability boundaries; construction details are in the source."
};

string[] infoAntiSymIntro = new[]
{
    "[[Anti-symmetric mode]] in RedX refers to asymmetric capabilities, not asymmetric encryption.",
...
{
    "Ciphertext encodes movement across a permutation, not substituted bytes.",
...
string[] infoIntegrityModeVsAnti = new[]
{
    ...
    "They can be used together: [[Anti-symmetric mode]] + integrity seal."
};

string[] infoKeysContain = new[]
{
    "Full key: permutation grid + hash + derivation material.",
    "ROK/verifier: key hash + per-position nonces + lookup map.",
    "Minting key: full key + authority signing keypair.",
    "Verifier key: ROK + authority public key only."
};
...
string[] infoAntiSymUseCases = new[]
{
...
string[] infoWireAntiSym = new[]
{
    "[[Anti-symmetric mode]] (integrity off): rokLock | enc(startLoc) | enc(ckCount) | enc(sigs) | intCatLen | enc(intCat) | [[Key-row offset stream]]",
    "[[Anti-symmetric mode]] (integrity on):  rokLock | enc(startLoc) | enc(ckCount) | enc(sigs) | intCatLen | enc(intCat) | [[Key-row offset stream]] | enc(seal)",
...
/// </summary>
void ShowAntiSymIntro(ref bool showAgain)
{
    Console.WriteLine();
    PrintBreadcrumb(BuildBreadcrumbPath("Anti-symmetric intro"));
    WriteLineColor(ConsoleColor.Cyan, "Anti-symmetric mode overview");
    foreach (var line in infoAntiSymIntro)
        WriteInfoLine(line);
...
void RunGlossaryIndex()
{
    while (true)
    {
        ...
        Console.WriteLine("  L) Anti-symmetric mode");
        ...
        else if (string.Equals(choice, "L", StringComparison.OrdinalIgnoreCase)) RunGlossaryAntiSymMenu();
...
void RunGlossaryCiphertextMenu()
{
    while (true)
    {
        ...
        Console.WriteLine("  G) Anti-symmetric mode");
        Console.WriteLine("  X) Back");
        ...
        else if (string.Equals(choice, "G", StringComparison.OrdinalIgnoreCase)) RunGlossaryAntiSymMenu();
    }
}
...
void RunGlossaryIntegrityMenu()
{
    while (true)
    {
        ...
        else if (string.Equals(choice, "D", StringComparison.OrdinalIgnoreCase)) ShowInfo("Integrity vs anti-symmetric", infoIntegrityModeVsAnti);
        ...
        else if (string.Equals(choice, "H", StringComparison.OrdinalIgnoreCase)) RunGlossaryAntiSymMenu();
    }
}
...
/// <summary>
/// Show short entries about anti-symmetric mode and its constraints.<br/>
/// </summary>
void RunGlossaryAntiSymMenu()
{
    while (true)
    {
        Console.WriteLine();
        WriteLineColor(ConsoleColor.Cyan, "Anti-symmetric mode");
        ...
        if (choice == "ESC" || string.Equals(choice, "X", StringComparison.OrdinalIgnoreCase)) return;
        if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase)) ShowInfo("Anti-symmetric: name and meaning", infoAntiSymName);
        else if (string.Equals(choice, "B", StringComparison.OrdinalIgnoreCase)) ShowInfo("Anti-symmetric use cases", infoAntiSymUseCases);
        else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase)) ShowInfo("Anti-symmetric limits", infoAntiSymLimits);
...
void RunGlossaryWireMenu()
{
    while (true)
    {
        ...
        else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase)) ShowInfo("Wire format: anti-symmetric", infoWireAntiSym);
        ...
        else if (string.Equals(choice, "I", StringComparison.OrdinalIgnoreCase)) RunGlossaryAntiSymMenu();
    }
}
...
/// </summary>
void RunSymmetricPreset(REKey key, bool useIntegrity)
{
    WriteLineColor(ConsoleColor.Cyan, "=== Symmetric pre-canned cases ===");
    foreach (var (label, plain) in BuildPresetPayloads())
    {
        var ct = REAnti.Encrypt(plain, key, useIntegrity);
        var ctBytes = ct.ToArray();
        var dec = REAnti.Decrypt(new BufferStream(ctBytes), key, useIntegrity);
...
/// </summary>
void SymmetricEncryptInteractive(REKey key, bool useIntegrity)
{
...
/// </summary>
void SymmetricDecryptInteractive(REKey key, bool requireIntegrity)
{
    ...
    try
    {
        dec = REAnti.Decrypt(new BufferStream(ctBytes), key, requireIntegrity);
    }
    catch (Exception ex)
    {
...
void SelectAntiSymKeys(ref RedXMintingKey minting, ref RedXVerifierKey verifier)
{
    while (true)
    {
        ...
        var choice = ReadChoiceKey("Select (A/B/C/?/X or ESC): ", "A", "B", "C", "?", "X");
        if (choice == "ESC" || string.Equals(choice, "X", StringComparison.OrdinalIgnoreCase)) return;
        if (string.Equals(choice, "?", StringComparison.OrdinalIgnoreCase)) { WithBreadcrumb("Key Options", () => RunGlossaryMenu(GlossaryContextAntiSymmetricKey)); continue; }
        if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase))
        {
...
/// </summary>
void RunAntiSymPreset(RedXMintingKey minting, RedXVerifierKey verifier, bool useIntegrity)
{
    WriteLineColor(ConsoleColor.Cyan, "=== Anti-symmetric pre-canned cases ===");
    foreach (var (label, plain) in BuildPresetPayloads())
    {
        var ct = REAnti.EncryptAntiSym(plain, minting, useIntegrity: useIntegrity);
        var ctBytes = ct.ToArray();
        BufferStream dec = null;
        try
        {
            dec = REAnti.DecryptAntiSymWithAuthority(new BufferStream(ctBytes), verifier, requireIntegrity: useIntegrity);
        }
...
/// </summary>
void AntiSymMintInteractive(RedXMintingKey minting, RedXVerifierKey verifier, bool useIntegrity)
{
    var plain = PromptPayloadBytes("plaintext");
    if (plain == null) return;
...
/// </summary>
void AntiSymVerifyInteractive(RedXVerifierKey verifier, bool requireIntegrity)
{
    var ctBytes = PromptPayloadBytes("ciphertext (hex preferred)");
    if (ctBytes == null) return;
    ...
    try
    {
        dec = REAnti.DecryptAntiSymWithAuthority(new BufferStream(ctBytes), verifier, requireIntegrity: requireIntegrity);
    }
    catch (InvalidDataException ex)
    {
        Console.WriteLine($" verify error: {ex.Message}");
...
void RunAntiSymMenu()
{
    breadcrumb = new List<string> { "Main", "Anti-symmetric" };
    if (showAntiSymIntro)
        ShowAntiSymIntro(ref showAntiSymIntro);
    RedXMintingKey minting = null;
    RedXVerifierKey verifier = null;
    bool useIntegrityAnti = true;
    ...
    while (true)
    {
        ...
        Console.WriteLine($"  A) Pre-canned demo (integrity:{(useIntegrityAnti ? "on" : "off")})");
        ...
        Console.WriteLine($"  ?) {GlossaryLabel}");
        Console.WriteLine("  M) Why \"anti-symmetric\"?");
        ...
        {
            useIntegrityAnti = !useIntegrityAnti;
            continue;
        }
        ...
        if (string.Equals(choice, "?", StringComparison.OrdinalIgnoreCase)) { RunGlossaryMenu(GlossaryContextAntiSymmetric); continue; }
        ...
        if (string.Equals(choice, "A", StringComparison.OrdinalIgnoreCase))
        {
            ...
            RunAntiSymPreset(minting, verifier, useIntegrityAnti);
        }
        else if (string.Equals(choice, "B", StringComparison.OrdinalIgnoreCase))
        {
            ...
            verifier ??= minting.CreateVerifierKey();
            AntiSymMintInteractive(minting, verifier, useIntegrityAnti);
        }
        else if (string.Equals(choice, "C", StringComparison.OrdinalIgnoreCase))
        {
            if (verifier == null)
            ...
            AntiSymVerifyInteractive(verifier, useIntegrityAnti);
        }
    }
}
